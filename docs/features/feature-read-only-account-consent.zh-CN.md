...
Athena 当前还缺少安全的 remote-tool secret reference 和 header 注入能力，跟踪于 [Super-Sky/Athena#24](https://github.com/Super-Sky/Athena/issues/24)。在该底座能力合入前，基金应用侧授权闭环可以独立验证，但真实 Athena 跨服务账户工具调用不能被视为生产可用。
...
服务端测试覆盖会话签发/撤销、跨用户拒绝、grant 创建/列表/撤销、服务身份拒绝、缺 scope 拒绝、授权成功、撤销后拒绝和原始凭据不落审计。
