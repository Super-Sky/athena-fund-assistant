// This file implements PostgreSQL journal and review persistence with embedded migrations.
// 本文件使用内嵌 migration 实现 PostgreSQL 决策日志与复盘持久化。
package journal
...
func (s *PostgresStore) Create(ctx context.Context, matrix domain.DecisionMatrix, selectedOptionID, notes string) (domain.JournalEntry, domain.ReviewTask, error) {
	...
	if _, err := tx.Exec(ctx, `INSERT INTO journal_entries (id, created_at, matrix_id, selected_option_id, snapshot) VALUES ($1, $2, $3, $4, $5)`, entry.ID, entry.CreatedAt, entry.MatrixID, entry.SelectedOptionID, entrySnapshot); err != nil {
		return domain.JournalEntry{}, domain.ReviewTask{}, fmt.Errorf("insert journal entry: %w", err)
	}
	if _, err := tx.Exec(ctx, `INSERT INTO review_tasks (id, journal_id, due_at, status, snapshot) VALUES ($1, $2, $3, $4, $5)`, review.ID, review.JournalID, review.DueAt, review.Status, reviewSnapshot); err != nil {
